...
class DiscreteDenoisingDiffusion(pl.LightningModule):
    ...
    def sample_batch(self, batch_id: int, batch_size: int, keep_chain: int, number_chain_steps: int,
        ...
        if num_nodes is None:
            n_nodes = self.node_dist.sample_n(batch_size, graph_target, graph_scaffold, self.device)
        ...
        X_target, E_target = graph_target.X, graph_target.E
        X_scaffold, E_scaffold = graph_scaffold.X, graph_scaffold.E

        n_nodes_target = X_target.shape[1]
        n_nodes_scaffold = X_scaffold.shape[1]

        while X.shape[1] < n_nodes_target + n_nodes_scaffold:
            n_remove = torch.randint(low = 0, high = n_nodes_target)
            X_target = torch.cat((X_target[:n_remove], X_target[n_remove+1:]), dim = 0)
            E_target = torch.cat((E_target[:n_remove], E_target[n_remove+1:]), dim = 0)
            E_target = torch.cat((E_target[:, :n_remove], E_target[:, n_remove+1:]), dim = 1)
            n_nodes_target -= 1
    ...
    def sample_p_zs_given_zt(self, s, t, X_t, E_t, y_t, node_mask, X_target, E_target, X_scaffold, E_scaffold, threshold):
        ...
        if t[0] >= threshold:
            ...
            sampled_s.X[:, :n_nodes_scaffold] = X_scaffold
            sampled_s.E[:, :n_nodes_scaffold, :n_nodes_scaffold] = E_scaffold
            sampled_s.X[:, n_nodes_scaffold:n_nodes_scaffold+n_nodes_target] = X_target
            sampled_s.E[:, n_nodes_scaffold:n_nodes_scaffold+n_nodes_target, n_nodes_scaffold:n_nodes_scaffold+n_nodes_target] = E_target
    ...
    def graph_from_scaffold(self, scaffold_smile):
        ...
        if self.cfg.dataset.name == 'qm9':
            if self.cfg.dataset.remove_h == True:
                types = {'C': 0, 'N': 1, 'O': 2, 'F': 3}
            else:
                types = {'H' : 0, 'C': 1, 'N': 2, 'O': 3, 'F': 4}
        elif self.cfg.dataset.name == 'moses':
            types = {'C' : 0, 'N' : 1, 'S' : 2, 'O' : 3, 'F' : 4, 'Cl' : 5, 'Br' : 6, 'H' : 7} 
